...
build_instruction: "Keep credentials in an OS credential reference supplied at operation time; never serialize secrets into the descriptor, manifest, cartridge, log, or event. The 2026-09-12 acquisition_entry_contract requires broker-owned source-connect triggers, provider-supported authorization, masked explicit token input where selected, OS-vault storage and reconnect/revocation handling through the product CLI; a callable credential_lookup alone does not prove onboarding."
acceptance_check: "Resolve one immutable fixture per source kind into the normalized output, expire credentials, move the cartridge, and prove the descriptor remains secret-free and source-specific code ends at the adapter boundary. Test the acquisition_entry_contract credential and CLI assertions through the actual supported launch; distinguish public access, provider-validated account connection and permitted model access."
depends_on: [Q1, Q8]
...
question_id: Q39
state: CLOSED
revised: 2026-09-11 under the principal's ungated sparse-model substitution
...
evidence:
  - status: OBSERVED
    result: "E-001 fixes K3; historical public Hugging Face APIs fixed Scout and Qwen revisions and reported 217,315,712,145 B and 470,211,497,053 B repository artifact totals respectively."
  - status: SPECIFIED
  ...
  - status: OBSERVED
    result: "The principal authorized replacing gated Scout. research/SPARSE_MODEL_SELECTION.json pins ungated Apache-2.0 Qwen3-30B-A3B@ad44e777bcd18fa416d9da3bd8f70d33ebb85d39 with 61084187391 repository artifact bytes. It retains the F5 20B-120B sparse class, existing C1 execution/training and Ollama obligations; no frontier model or gate is replaced."
...
question_id: Q52
state: CLOSED
decision: "A source adapter has six operations and owns no lifecycle state beyond authentication translation. The principal's 2026-09-12 catalogue-selection requirement admits discover as the sixth operation; the prior five-operation proof remains historical evidence for its original scope."
formal_contract:
  symbols: "Adapter={discover(query,cursor?,credential_ref?)->{candidates,next_cursor?}, resolve(SourceDescriptor)->ResolvedSource, enumerate(revision)->Artifact[], read_metadata(revision,ranges?)->MetadataEvidence, open_range(artifact,offset,length,validator)->ByteStream, license_and_auth(revision)->Requirements}; candidates contain source locators and declared access/downloadability, not an execution verdict. Selection resolves an immutable identity through resolve. All outputs use generated Q1/Q9/Q50 schemas and canonical errors."
...
reopen_only_if: "A required source behavior cannot be expressed by discovery or the five immutable-artifact operations. The 2026-09-12 discovery amendment remains unproved until acquisition_entry_contract passes."
```
