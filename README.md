...
representation, executes it through Apple Silicon with a bounded working set in unified memory, and
...
> [!IMPORTANT]
> Cassette is being built in public. S01–S05 are complete: the repository has its accounting and
> error authorities, generated protocol schemas, canonical model identity, and the first
> content-addressed SafeTensors cartridge store. It does **not** yet download, compile, execute,
> train, or serve a model. The complete release matrix remains `NOT_RUN`.
...
4. Cassette chooses the least invasive representation that passes the declared memory, performance,
   capability, and quality gates. If native execution cannot pass, Cassette compiles a separately
   identified storage-native revision whose complete relationship to the source remains provable.
5. During inference, the cartridge retains authority over the model while Apple unified memory holds
   only the admitted parameter working set, cache, context state, activations, and reserve.
...
flowchart LR
    ...
    subgraph MAC["macOS and Apple Silicon"]
        ...
        U["Bounded unified memory<br/>working set, cache, KV, activations"] <--> P
...
post-trained prompt-persistent compiled revision. A native model is never transformed merely
because transformation is interesting; an earlier mode must fail its acceptance contract first.

For a compiled layer, the research contract permits a shared core plus request-selected parameter
pages:

$$
W_l^{(r)} = W_l^0 + \sum_{j \in R_l(r)} \Delta W_{lj}
$$

The request chooses $R(r)$ before decoding and keeps it stable except for typed corrective misses.
Training may penalize bytes loaded, seeks, page churn, memory violations, and quantization error in
addition to teacher disagreement. This is one admitted mechanism, not a promise that every model
will use it; a model with compatible native sparsity should keep its native semantics.

Two physical inequalities govern every plan:

$$
W(r,t) + C(t) + K(r,t) + A(t) + R \le M
$$
...
The first keeps the working set, cache, context, activations, and reserve inside the admitted unified
memory budget. The second refuses to call a plan fast when storage traffic, memory traffic, or
...
token at impossible 100% utilization, or about 10.7 GB at 70%. Cassette must change the active path;
...
LoRA or adapter SFT, offline DPO, adapter continued pretraining, prompt-router recovery, and
precision-tier recovery. Each job must reserve storage, memory, sustained bandwidth, write
endurance, power, thermal duty, and completion time before it changes a byte.
...
The repository has finished five of twenty-eight machine-build steps. The present code is the
...
| S06 | Next | Transaction journal, durable root generations, process-kill injection, and remount proof on macOS. |

The latest committed proof on Python 3.13 reports 17 passing tests and a clean ledger: 540 product
lines, 324 tool lines, 350 test lines, 58 generated lines reported separately, one process, one
authored language, and three exact direct dependency records including the test dependency. The S05
closeout also records an independent `safetensors==0.6.2` writer probe that recovered 4,194,311
bytes exactly.

Current boundaries are explicit. SafeTensors import exists only at the cartridge-store layer;
whole-revision source acquisition, GGUF import, export, appended training deltas, atomic generation
selection, repair, paging, MLX execution, compilation, training, brokering, named-client adapters,
and any UI do not yet exist. Fields reserved for later root material do not count as those
operations.
...
3. **S12–S15 — execution:** pinned runtime dispatch, memory schedules, page readiness and miss
   recovery, followed by a tiny end-to-end transformer from a cartridge.
...
5. **S19–S20 — compilation:** streaming transforms, complete contribution maps, hardware plans, and
   invalidation without duplicate weights.
6. **S21–S22 — training:** paged on-cartridge optimization, metering, and admission.
7. **S23–S26 — falsification:** generated failure injection, teacher traces, a prompt-persistent
   3–8B build, retention-versus-compression curves, and the F4 gate.
...
permissively licensed 3–8B dense model to preserve at least 95% of teacher quality while touching no
more than one quarter of its native active bytes under a prompt-persistent plan. F5 repeats the
predicates at 20–120B scale and must predict a frontier point inside the consumer decode budget.
...
format, inference, working-set routing, training, durability, protocols, minimum code, provenance,
...
Eleven evidence records separate public artifact observations, specifications, measurements,
...
Cassette currently pins Python `==3.13.*`, `blake3==1.0.9`, `rfc8785==0.1.4`, and the development
dependency `pytest==9.1.1`. The following commands reproduce the same isolated checks used for this
README:
...
Cassette is queue-built. Before changing code, read the remit, research ledger, acceptance matrix,
build rules, and implementation queue; select the first eligible `TODO` step; name the exact
