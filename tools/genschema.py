...
def adapter_field(
    ...
    *,
    ...
    field = {
        "canonical": [int(part) if part.isdecimal() else part for part in canonical.split(".")],
        "wire": [int(part) if part.isdecimal() else part for part in wire.split(".")],
        "codec": codec,
    }
    if mirrors:
        field["mirrors"] = [[int(part) if part.isdecimal() else part for part in path.split(".")] for path in mirrors]
    return field
...
# Q31/Q76: provider-required envelope fields accompany the reversible event mapping.
for event_name, event_spec in ADAPTER_EVENT_FORMATS["chat_sse"]["events"].items():
    event_spec["fields"].extend([
        adapter_field("payload.created", "created"),
    ])
    event_spec["fields"][0] = adapter_field("run_id", "x_cassette.run_id", mirrors=("id",))
    if event_name != "started":
        event_spec["fields"].append(adapter_field("payload.model_ref", "model", "model"))
    event_spec["static"].update({"choices.0.index": 0, "choices.0.delta": {}})
    if event_name != "completed":
        event_spec["static"]["choices.0.finish_reason"] = None
    if event_name == "started":
        event_spec["static"]["choices.0.delta.role"] = "assistant"
    if event_name == "tool_call":
        event_spec["static"].update({"choices.0.delta.tool_calls.0.index": 0,
                                    "choices.0.delta.tool_calls.0.type": "function"})
for event_name in ("started", "completed", "failed"):
    event_spec = ADAPTER_EVENT_FORMATS["responses_sse"]["events"][event_name]
    event_spec["fields"].append(adapter_field("payload.created", "response.created_at"))
    if event_name != "started":
        event_spec["fields"].append(adapter_field("payload.model_ref", "response.model", "model"))
    event_spec["static"].update({"response.object": "response", "response.output": [],
                                "response.parallel_tool_calls": False, "response.tools": [],
                                "response.tool_choice": "none", "response.metadata": {}})
ADAPTER_EVENT_FORMATS["responses_sse"]["events"]["completed"]["static"].pop("response.output")
ADAPTER_EVENT_FORMATS["responses_sse"]["events"]["completed"]["fields"].append(
    adapter_field("payload.response_output", "response.output"))
for event_name in ("output_delta", "reasoning_delta"):
    event_spec = ADAPTER_EVENT_FORMATS["responses_sse"]["events"][event_name]
    event_spec["fields"][0] = adapter_field("run_id", "x_cassette.run_id", mirrors=("item_id",))
    event_spec["static"].update({"output_index": 0, "content_index": 0})
    if event_name == "output_delta":
        event_spec["static"]["logprobs"] = []
...
NATIVE_OBJECTIVES = {
    ...
    "primitives": ["mlx.core.value_and_grad", "mlx.core.logsumexp", "mlx.core.take_along_axis",
                   "mlx.core.logaddexp", "mlx.core.sum", "mlx.core.matmul", "mlx.core.stop_gradient", "mlx.core.eval", "mlx.optimizers.SGD"],
}
...
HARDWARE_PROFILE_PREDICATE = record(
    ...
    "Q11/Q33/Q59",
    {
        ...
        "operation_plan_profile_digest": digest(),
...
def emit(outdir: Path) -> dict[str, str]:
    ...
    queue_text = (Path(__file__).resolve().parent.parent / "IMPLEMENTATION.md").read_text()
    ...
    campaign_contract["assertion_owner_patterns"] = {
        "baseline_freeze": ["BASELINES-ALL-PASS"],
        "adaptive_capacity_control": ["L04"],
        "storage_eligibility_control": ["L04"],
        ...
        "workload_suites[id=capability_complete_q16]": ["EXEC-.+-RUN_VERIFY"],
        "workload_suites[id=sustained_q48]": ["SUSTAINED-.+-VERIFY"],
        "protocol_cross_product": ["PROTOCOL-.+"],
        "training_assertions": ["TRAIN-.+-(adapter_sft_[0-9]+_tokens|adapter_continued_pretraining_[0-9]+_tokens|offline_adapter_dpo_[0-9]+_pairs)-VERIFY"],
        "failure_rows": ["FAILURE-.+-VERIFY"],
        "offline_and_privacy_rows[id=offline_inference_all_execution_rows]": ["OFFLINE-INFERENCE-.+-VERIFY"],
    ...
    campaign_contract["matrix_axes"] = {
        "training": {"row": "training_rows"}, "execution": {"row": "execution_rows"},
        "sustained": {"row": "execution_rows"}, "protocol": {"row": "execution_rows", "client": "agent_clients"},
        "offline_inference": {"row": "execution_rows"}, "offline_training_audit": {"row": "training_rows"},
    }
    (outdir / "campaign").mkdir(exist_ok=True)
    put("campaign/contract.json", json.dumps(campaign_contract, sort_keys=True, indent=2) + "\n")
